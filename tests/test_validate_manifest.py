...
# ═══════════════════════════════════════════════════════════════════════════
# CANONICAL ENTITY: kind-aware validation
# ═══════════════════════════════════════════════════════════════════════════


def _write_entity(directory: Path, manifest_version: str) -> Path:
    """Write a minimal canonical_entity manifest + schema into directory."""
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "schema.avsc").write_text(
        json.dumps(
            {"type": "record", "name": "E", "fields": [{"name": "x", "type": "string"}]}
        ),
        encoding="utf-8",
    )
    manifest = {
        ...
        "status": "active",
        "metadata": {"name": "customer", "namespace": "party", "owner": {"team": "t", "email": "e@x"}},
        "schema": {"file": "./schema.avsc", "format": "avro"},
        "changelog": [{"version": manifest_version}],
    }
...
class TestCanonicalEntity:
    def test_entity_does_not_require_product_sections(self, tmp_path):
        """A canonical entity needs no semantics/quality_rules/sla/lineage."""
        path = _write_entity(tmp_path / "v1", "1.0.0")
        errors = validate_required_fields(path)
        assert errors == []

    def test_version_dir_matches_major(self, tmp_path):
        path = _write_entity(tmp_path / "v2", "2.0.0")
        errors = validate_required_fields(path)
        assert errors == []

    def test_version_dir_mismatch_is_error(self, tmp_path):
        path = _write_entity(tmp_path / "v2", "1.0.0")
        errors = validate_required_fields(path)
        assert any("must have manifest_version major 2" in e for e in errors)

    def test_invalid_kind_is_error(self, tmp_path):
        path = _write_entity(tmp_path / "v1", "1.0.0")
