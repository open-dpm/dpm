...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["validate-conformance", "m.yaml", "--registry-path", "canonical"],
            "dpm.validators.validate_conformance.main",
        ),
        (
            ["validate-conformance", "--all", "--registry-path", "canonical"],
            "dpm.validators.validate_conformance.main",
        ),
        (
...
@pytest.mark.parametrize("command", ["validate", "governance"])
def test_cli_all_runs_end_to_end_on_examples(command, repo_root):
    """`--base-path` is forwarded and accepted by the validator (no argparse error).

    Runs the real validator over the bundled examples; a wrong flag name would
    surface as SystemExit(2) from argparse instead of a clean 0.
    """
    with pytest.raises(SystemExit) as exc:
        cli.main([command, "--all", "--base-path", str(repo_root)])
    assert exc.value.code == 0


def test_cli_validate_conformance_requires_path_or_all(mocker):
    """validate-conformance needs a path or --all even with a registry."""
    mocker.patch("dpm.validators.validate_conformance.main")
    with pytest.raises(SystemExit):
        cli.main(["validate-conformance", "--registry-path", "canonical"])
